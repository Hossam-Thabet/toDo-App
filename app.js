...
function addTask(title){
  ...
  const task = { id: meta.nextId++, title, done:false, created: Date.now() };
...
function applyTranslations(){
  ...
  if(statsEl){
    const tasks = getTasks();
    statsEl.textContent = t('stats', tasks.filter(x=>x.done).length, tasks.length);
  }
}
...
function initHomePage() {
  ...
  function render() {
    ...
    tasks.forEach(task => {
      ...
      item.innerHTML = `
        ...
        <div class="task-actions">
          <button class="btn ghost edit" data-id="${task.id}">${t('editBtn')}</button>
          <button class="btn ghost delete" data-id="${task.id}">${t('deleteBtn')}</button>
        </div>
      `;
    ...
    if(statsNode) statsNode.textContent = t('stats', getTasks().filter(x=>x.done).length, getTasks().length);
  }
...
function initLoginPage() {
  ...
  formLogin.addEventListener('submit', (ev) => {
    ...
    if(!r.ok){ alert(t('invalidCreds')); return; }
